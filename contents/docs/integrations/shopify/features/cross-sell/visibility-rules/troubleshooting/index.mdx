...
<Step>
  ...
  <StepItem title="Tag Name Mismatch">
    **Check:** Product tag value EXACTLY matches customer tag (case-sensitive)

    **Fix:** Correct spelling/capitalization in product tag or customer tag

    **Common mistakes:**
    ```
    Product tag: hide_if_customer_tag==VIP
    Customer tag: "vip"
    Result: No match (case difference)

    Fix: Change customer tag to "VIP" OR change product tag to ==vip
    ```
  </StepItem>
...
**Fix:** Match case exactly: `VIP` or `vip` in both places
...
- **Tag format reference:** [Tag Reference](/docs/integrations/shopify/features/cross-sell/visibility-rules/tag-reference)
- **Setup instructions:** [Setup Guide](/docs/integrations/shopify/features/cross-sell/visibility-rules/setup)
- **Use case examples:** [Examples](/docs/integrations/shopify/features/cross-sell/visibility-rules/examples)
